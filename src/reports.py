...
def report_by_category(transactions: DataFrame, category: str, date: Optional[str] = None) -> object:
    ...
    filtered_by_date = transactions[
        (date_to_start_str <= transactions["Дата операции"]) & (transactions["Дата операции"] <= date_datetime_str)
    ]
    filtered_by_category = filtered_by_date[
        (filtered_by_date["Категория"] == category) & (filtered_by_date["Сумма платежа"] < 0)
    ]
