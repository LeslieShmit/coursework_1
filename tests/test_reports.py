...
def test_report_by_category(df_unfiltered):
    ...
    assert report_by_category(pd.DataFrame(df_unfiltered), "Красота", "2018-02-04 09:30:12") == json.dumps(
        pd.DataFrame(expected_result).to_dict("records"), indent=4, ensure_ascii=False
    )
